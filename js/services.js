...
const ProdutosService = {
  async listar(filtros = {}) {
    let rows = await tableQuery("JuliaProdutos");
    if (filtros.marca)
      rows = rows.filter((r) =>
  ...
  async buscarPorId(id) {
    const rows = await tableQuery("JuliaProdutos", `RowKey eq '${id}'`);
    return rows[0] || null;
  },
  ...
  async salvar(produto, imagemFile = null) {
    ...
    await tableRequest("JuliaProdutos", "POST", entity);
    return entity;
  },
  ...
  async atualizar(id, produto, imagemFile = null) {
    ...
    const url = `${AZURE_CONFIG.tableEndpoint}/JuliaProdutos(PartitionKey='Produto',RowKey='${encodeURIComponent(id)}')?${AZURE_CONFIG.sasToken}`;
...
const ClientesService = {
  async listar() {
    return tableQuery("JuliaClientes");
  },

  async buscarPorId(id) {
    const rows = await tableQuery("JuliaClientes", `RowKey eq '${id}'`);
    return rows[0] || null;
  },
  ...
  async salvar(cliente) {
    ...
    await tableRequest("JuliaClientes", "POST", entity);
    return entity;
  },
  ...
  async atualizar(id, cliente) {
    ...
    const url = `${AZURE_CONFIG.tableEndpoint}/JuliaClientes(PartitionKey='Cliente',RowKey='${encodeURIComponent(id)}')?${AZURE_CONFIG.sasToken}`;
    await fetch(url, {
      method: "PUT",
      headers: {
        Accept: "application/json;odata=nometadata",
  ...
  async historicoPedidos(clienteId) {
    return tableQuery("JuliaPedidos", `ClienteId eq '${clienteId}'`);
  },
};
...
const PedidosService = {
  async listar() {
    return tableQuery("JuliaPedidos");
  },

  async buscarPorId(id) {
    const rows = await tableQuery("JuliaPedidos", `RowKey eq '${id}'`);
    return rows[0] || null;
  },
  ...
  async criar(pedido) {
    ...
    await tableRequest("JuliaPedidos", "POST", entity);
