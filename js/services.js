...
const ProdutosService = {
  async listar(filtros = {}) {
    ...
    if (filtros.marca) rows = rows.filter(r => r.Marca?.toLowerCase().includes(filtros.marca.toLowerCase()));
    if (filtros.modelo) rows = rows.filter(r => r.Modelo?.toLowerCase().includes(filtros.modelo.toLowerCase()));
    if (filtros.precoMin) rows = rows.filter(r => parseFloat(r.Valor) >= parseFloat(filtros.precoMin));
    if (filtros.precoMax) rows = rows.filter(r => parseFloat(r.Valor) <= parseFloat(filtros.precoMax));
    return rows;
  },
  ...
  async salvar(produto, imagemFile = null) {
    const id = produto.id || `prod_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;
  ...
  async atualizar(id, produto, imagemFile = null) {
    ...
    if (!res.ok && res.status !== 204) throw new Error(`Atualizar produto: ${res.status}`);
    return entity;
  },
 ...
 async excluir(id) {
  ...
  const url = `${AZURE_CONFIG.tableEndpoint}/JuliaProdutos(PartitionKey='${encodeURIComponent(prod.PartitionKey)}',RowKey='${encodeURIComponent(id)}')?${AZURE_CONFIG.sasToken}`;
  const res = await fetch(url, {
    method: 'DELETE',
    headers: {
      'x-ms-version': '2020-12-06',
  ...
  async atualizarQuantidade(id, delta) {
    ...
    await this.atualizar(id, {
      marca: prod.Marca, modelo: prod.Modelo, valor: prod.Valor,
      quantidade: prod.Quantidade, descricao: prod.Descricao, imagemUrl: prod.ImagemUrl, ativo: prod.Ativo,
    });
    return prod;
  },
};
...
const ClientesService = {
  ...
  async atualizar(id, cliente) {
    ...
    const entity = {
      ...
      IsSeller: cliente.isSeller !== undefined ? cliente.isSeller : (existing?.IsSeller || false),
    };
  ...
  async criar(pedido) {
    ...
    for (const item of pedido.itens) {
      ...
      if (parseInt(prod.Quantidade) < item.quantidade)
        throw new Error(`Estoque insuficiente para ${prod.Modelo}: disponível ${prod.Quantidade}, solicitado ${item.quantidade}`);
    }
    ...
    // Deduct stock
    for (const item of pedido.itens) {
      await ProdutosService.atualizarQuantidade(item.produtoId, -item.quantidade);
    }
...
const CartService = {
  ...
  add(produto, quantidade = 1) {
    ...
    else cart.push({ produtoId: produto.RowKey, nome: `${produto.Marca} ${produto.Modelo}`, valor: parseFloat(produto.Valor), quantidade, imagemUrl: produto.ImagemUrl });
    this.save(cart);
    updateCartBadge();
  },
  remove(produtoId) { this.save(this.get().filter(i => i.produtoId !== produtoId)); updateCartBadge(); },
  clear() { localStorage.removeItem('cart'); updateCartBadge(); },
  total() { return this.get().reduce((s, i) => s + i.valor * i.quantidade, 0); },
  count() { return this.get().reduce((s, i) => s + i.quantidade, 0); },
};
...
const SessionService = {
  getCliente() { try { return JSON.parse(localStorage.getItem('clienteLogado') || 'null'); } catch { return null; } },
  setCliente(c) { localStorage.setItem('clienteLogado', JSON.stringify(c)); },
  logout() { localStorage.removeItem('clienteLogado'); },
  isLogged() { return !!this.getCliente(); },
};
